...
function optionTakerBlockVolume(ccy, period) {
    if (!ccy) {
        return { status: 400, message: 'ccy is required' };
    }
...
function optionOpenInterestVolumeStrike(ccy, expTime, period) {
    if (!ccy) {
        return { status: 400, message: 'ccy is required' };
    }
    if (!expTime) {
        return { status: 400, message: 'expTime is required' };
    }
...
function optionOpenInterestVolumeExpiry(ccy, period) {
    if (!ccy) {
        return { status: 400, message: 'ccy is required' };
    }
    let params = [`ccy=${encodeURIComponent(ccy)}`];
...
function optionOpenInterestVolumeRatio(ccy, period) {
    if (!ccy) {
        return { status: 400, message: 'ccy is required' };
    }
    let params = [`ccy=${encodeURIComponent(ccy)}`];
...
function optionOpenInterestVolume(ccy, period) {
    if (!ccy) {
        return { status: 400, message: 'ccy is required' };
    }
    let params = [`ccy=${encodeURIComponent(ccy)}`];
...
function openInterestVolume({ ccy, begin, end, period }) {
    if (!ccy) {
        return { status: 400, message: 'ccy is required' };
    }
    let params = [`ccy=${encodeURIComponent(ccy)}`];
    if (begin)
        params.push(`begin=${encodeURIComponent(begin)}`);
    if (end)
...
function longShortAccountRatio(ccy, begin, end, period) {
    if (!ccy) {
        return { status: 400, message: 'ccy is required' };
    }
    let params = [`ccy=${encodeURIComponent(ccy)}`];
...
function longShortAccountRatioContract(instId, period, end, begin, limit) {
    if (!instId) {
    ...
    return CallHttp(
        `contracts/long-short-account-ratio-contract?${params.join('&')}`
    );
}

function longShortPositionRatioContractTopTrader(instId, period, end, begin, limit) {
    if (!instId) {
        return { status: 400, message: 'instId is required' };
    }
    let params = [`instId=${encodeURIComponent(instId)}`];
    ...
    if (limit)
        params.push(`limit=${encodeURIComponent(limit)}`);
    return CallHttp(
        `contracts/long-short-position-ratio-contract-top-trader?${params.join('&')}`
    );
}

function longShortAccountRatioContractTopTrader(instId, period, end, begin, limit) {
    if (!instId) {
        return { status: 400, message: 'instId is required' };
    }
    let params = [`instId=${encodeURIComponent(instId)}`];
    ...
    if (limit)
        params.push(`limit=${encodeURIComponent(limit)}`);
    return CallHttp(
        `contracts/long-short-account-ratio-contract-top-trader?${params.join('&')}`
    );
}

function marginLoanRatio(ccy, begin, end, period) {
    if (!ccy) {
        return { status: 400, message: 'ccy is required' };
    }
    let params = [`ccy=${encodeURIComponent(ccy)}`];
...
function takerVolumeContract(instId, period, unit, end, begin, limit) {
    if (!instId) {
        return { status: 400, message: 'instId is required' };
    }
    let params = [`instId=${encodeURIComponent(instId)}`];
...
function takerVolume(ccy, instType, begin, end, period) {
    if (!ccy) {
        return { status: 400, message: 'ccy is required' };
    }
    if (!instType) {
...
function contractsOpenInterestHistory(instId, period, end, begin, limit) {
    if (!instId) {
        return { status: 400, message: 'instId is required' };
    }
    let params = [`instId=${encodeURIComponent(instId)}`];
